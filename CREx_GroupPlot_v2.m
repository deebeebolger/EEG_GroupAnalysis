...
if length(cfg.Condnom)==2   %if there are only two conditions
    ...
    for ecounter=1:length(cfg.eoi)
        ...
        set(Axes(ecounter),'HitTest','on','SelectionHighlight','on','UserData',{Time,X1,X2,X1mean,X2mean,colrs,cfg.eoi{1,ecounter},gfp_cond1,gfp_cond2,GMD,cfg.Condnom,cfg.ylims},'NextPlot','new');
